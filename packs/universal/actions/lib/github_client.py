...
class GitHubClient:
    ...
    def clone_repo(self, repo_url: str, branch: str) -> Path:
        ...
        tmpdir = Path(tempfile.mkdtemp(prefix="st2_universal_git_"))
        LOG.info(
    ...
    def resolve_under_repo(self, repo_root: Path, relative_path: str) -> Path:
        """
        Resolve a repository-relative path under ``repo_root/repo`` with traversal containment.

        Raises:
            GitHubClientError: If the path escapes the clone root.
        """
    ...
    def get_repo_file(
        ...
        """
        Resolve a file under the clone and optionally enforce filename suffixes.

        Args:
            repo_root: Temporary directory returned by :meth:`clone_repo`.
            relative_path: Path relative to repository root.
            allowed_suffixes: If set, ``candidate.suffix`` must be lowercase member (e.g. {'.yaml'}).
        """
    ...
    def get_script_path(self, repo_root: Path, script_path: str) -> Path:
        """
        Resolve ``script_path`` under ``repo_root/repo`` and enforce containment.

        Args:
            repo_root: Temporary directory returned by :meth:`clone_repo` (contains ``repo``).
            script_path: Repository-relative path to the ``.sh`` file.

        Returns:
            Absolute Path to the script inside the repository.

        Raises:
            GitHubClientError: If path escapes the repo or is not a file.
        """
