...
Hostfiles map logical names to connection targets. No secrets are read from
these files — use Delinea for credentials.
"""
...
def load_hostfile(path: Path) -> Dict[str, Any]:
    ...
    The top-level structure must be a JSON object / YAML mapping (dict).

    Raises:
        HostfileError: On unsupported extension, parse errors, or invalid shape.
    """
...
def resolve_host_entry(data: Dict[str, Any], host_entry: str) -> Tuple[str, Optional[int]]:
    ...
    - Short form: ``my-host: "10.0.0.5"``
    - Mapping: ``my-host: { host: 10.0.0.5, port: 22 }`` (also accepts
      ``hostname``, ``ip``, ``address``, ``ssh_port``).
    """
